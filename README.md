...
`tui.json`, which adds a small sidebar showing specialist-agent status,
active/reusable task sessions, and orchestrating context telemetry (`ctx`,
input/output, cache read/write) per visible session entry. For manual setups,
add `oh-my-opencode-slim` to the `plugin` array in both `opencode.json` and
`tui.json`.
